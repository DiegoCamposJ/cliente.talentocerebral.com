...
trait EvaluacionTrait
{
    ...
    public function variablesNucleoQUAD($id)
    {
        ...
        $v1 = round(((($azConsolidadoNucleo * $CONS_QUAD) / $sumaConsolidadoNucleo)),0);
        $v2 = round(((($veConsolidadoNucleo * $CONS_QUAD) / $sumaConsolidadoNucleo)),0);
        $v3 = round(((($roConsolidadoNucleo * $CONS_QUAD) / $sumaConsolidadoNucleo)),0);
        $v4 = round(((($amConsolidadoNucleo * $CONS_QUAD) / $sumaConsolidadoNucleo)),0);

        $sumaValores = $v1 + $v2 + $v3 + $v4;


        if ($sumaValores < $CONS_QUAD )
        {
            $difValores = $CONS_QUAD - $sumaValores;

            $maxValor   = max( abs($v1), abs($v2), abs($v3), abs($v4));
            $ajuste = false;
            //dd($maxValor);

            if (abs($v1) == abs($maxValor) && $ajuste == false)
            {
                $v1 = $v1 + $difValores;
                $ajuste = true;
            ...
            if (abs($v2) == abs($maxValor) && $ajuste == false)
            {
                $v2 = $v2 + $difValores;
                $ajuste = true;
            ...
            if (abs($v3) == abs($maxValor) && $ajuste == false)
            {
                $v3 = $v3 + $difValores;
                $ajuste = true;
            ...
            if (abs($v4) == abs($maxValor) && $ajuste == false)
            {
                $v4 = $v4 + $difValores;
                $ajuste = true;
    ...
    {
        //dd($FCOLOR1, $FCOLOR2,  $FCOLOR3,  $FCOLOR4);
        ...
        IF ($FCOLOR1 == "RO"  && $FCOLOR2 == "AZ" &&  $FCOLOR3 == "AM" && $FCOLOR4 == "VE")
            $flujo = "SEMICRUZADO";
    ...
    public function porcentajeCargo($evaluacion,$campana)
    {
        $CONS_QUAD = 295;
        $cargo = DB::table('cargos')
        ->select('cargos.*')
        ->where('id',$campana->cargo_id)
        ->first();

        $tmpEvaluacion = 100 - ( ( ( abs($evaluacion->v1 - $cargo->res_cuadrantea) +  abs($evaluacion->v2 - $cargo->res_cuadranteb) + abs($evaluacion->v3 - $cargo->res_cuadrantec) + abs($evaluacion->v4 - $cargo->res_cuadranted)) / $CONS_QUAD) * 100);

        DB::table('evaluaciones')
            ->where('id', $evaluacion->id)
            ->update([  'porcentaje_cargo' => $tmpEvaluacion]);
    ...
    public function reporteQUAD($id)
    {
        ...
        if($HEM_IZQUIERO >0 && $HEM_DERECHO >0)
        {
            $HEM_DERECHO = round((100 - $HEM_IZQUIERO),2);
        }
       ...
       // dd($COD_AZ . ' ' . $COD_VE . ' ' . $COD_RO . ' ' . $COD_AM);
        ...
        //     ->where('id', $id)
        //     ->update(['h_izqquierdo' => $HEM_IZQUIERO,
        //               'h_derecho'    => $HEM_DERECHO,
        //               'neocortex'    => $NEOCORTEX,
        //               'limbica'      => $LIMBICA,

        //               'flujo'        => $FLUJO_PENSAMIENTO,
        //               'color'        => $FCOLOR4,
        //               'puntaje1'      => $FL4,
        //               'puntaje2'      => $FL3,
        //               'puntaje3'      => $FL2,
        //               'puntaje4'      => $FL1,
        //               'codigo'       => ($COD_AZ.$COD_VE.$COD_RO.$COD_AM),
        //               'f1x'          => $P1X,
        //               'f1y'          => $P1Y,
        //               'f2x'          => $P2X,
        //               'f2y'          => $P2Y,
        ...
        $datos = [
            'fechareporte'      => $fecha->isoFormat('DD MMMM  YYYY'),
            'nombre'            => $evaluacion->nombre,
            'nombreCompleto'    => $evaluacion->nombre . ' ' .$evaluacion->apellido,
        ];


            //($HEM_IZQUIERO);
            //dd($FLUJO_PENSAMIENTO);
            ...
            return view('quad.reporte_quad',
                ['datos' => $datos,
                 ...
                 'COD_CE' => ($COD_AZ.$COD_VE.$COD_RO.$COD_AM),
                 'codigos' => $codigos[0],
                 'P1X' => $P1X, 'P1Y' => $P1Y,
                 'P2X' => $P2X, 'P2Y' => $P2Y,
                 'P3X' => $P3X, 'P3Y' => $P3Y,
    ...
    public function reporteQUADEJE($id)
    {
        ...
        if(($NEOCORTEX + $LIMBICA) < 100)
        {
            ...
            return view('quad.reporte_quadejecutivo',
                ['evaluacion'=>$evaluacion,
                 'fecha' => $fecha->isoFormat('DD MMMM  YYYY'),
                 'HEM_IZQUIERO' =>   $HEM_IZQUIERO,
                 'HEM_DERECHO'  =>   $HEM_DERECHO,
    ...
    public function reporteQUADCARGO($id)
    {
        ...
        //----INFORMAICON DEL CARGO, se remplaza por el circustancial
        //dd('cargo',$cargo);
        $PT_CIRCUN_AZ = $cargo->res_cuadrantea;
        $PT_CIRCUN_VE = $cargo->res_cuadranteb;
        $PT_CIRCUN_RO = $cargo->res_cuadrantec;
        $PT_CIRCUN_AM = $cargo->res_cuadranted;
       ...
       // dd($COD_AZ . ' ' . $COD_VE . ' ' . $COD_RO . ' ' . $COD_AM);
        ...
        if(($NEOCORTEX + $LIMBICA) < 100)
        {
            ...
            return view('quad.reporte_quadcargo',
                ['evaluacion'=>$evaluacion,
                 'fecha' => $fecha->isoFormat('DD MMMM  YYYY'),
                 'HEM_IZQUIERO' =>   $HEM_IZQUIERO,
                 'HEM_DERECHO'  =>   $HEM_DERECHO,
    ...
    public function reporteDEMO($id)
    {
        $CONS_QUAD = 295;
        $fecha = Carbon::now();
        $evaluacion = DB::table('evaluaciones')
       ...
       // dd($COD_AZ . ' ' . $COD_VE . ' ' . $COD_RO . ' ' . $COD_AM);
        ...
        if ($evaluacion->carrera_preferida > 0)
        {
            ...
            return view('quad.reporte_quaddemo',
                ['evaluacion'=>$evaluacion,
                 'fecha' => $fecha->isoFormat('DD MMMM  YYYY'),
                 'HEM_IZQUIERO' =>   $HEM_IZQUIERO,
                 'HEM_DERECHO'  =>   $HEM_DERECHO,
